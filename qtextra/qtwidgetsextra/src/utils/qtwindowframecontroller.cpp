...
{
    if (d->target)
        updateCursorShape(d->target->mapToGlobal(event->position().toPoint()));
    return false;
}
...
bool QtWindowFrameController::mousePress(QMouseEvent* event)
{
    ...
    d->calculateCursorPosition(event->globalPosition().toPoint(), d->target->frameGeometry(), d->mousePress);
    ...
    {
        d->dragStarted = true;
        d->dragPos = event->globalPosition().toPoint();
    }
...
bool QtWindowFrameController::mouseMove(QMouseEvent* event)
{
    ...
    const QRect rc = d->evaluateGeometry(event->globalPosition().toPoint());
